/**
 * Digital Node Nexus Panel
 * A Home Assistant custom panel for controlling ESP32 devices via MQTT.
 *
 * Place this file at:
 *   config/www/digital_node_nexus/digital-node-nexus-panel.js
 *
...
class ESP32CommanderPanel extends HTMLElement {
  ...
  async _initAsync() {
    ...
    await this._loadPlacementLists();
  }

  async _loadPlacementLists() {
    if (!this._hass) return;
    const areaSel = this.shadowRoot.getElementById("place-area");
    const zoneSel = this.shadowRoot.getElementById("place-zone");
    try {
      const areas = await this._hass.connection.sendMessagePromise({
        type: "digital_node_nexus/list_areas",
      });
      if (areaSel) {
        const current = areaSel.value;
        areaSel.innerHTML = `<option value="">”” none ””</option>` +
          (areas.areas || []).map((a) =>
      ...
      console.warn("[Digital Node Nexus] list_areas failed", err);
    }
    try {
      const zones = await this._hass.connection.sendMessagePromise({
        type: "digital_node_nexus/list_broadcast_zones",
      });
      if (zoneSel) {
        const current = zoneSel.value;
        zoneSel.innerHTML = `<option value="">”” none ””</option>` +
          (zones.zones || []).map((z) =>
            `<option value="${z.zone_id}">${z.name || z.zone_id} (${z.zone_id})</option>`
          ).join("");
        if (current) zoneSel.value = current;
      }
    } catch (err) {
      console.warn("[Digital Node Nexus] list_broadcast_zones failed", err);
    }
    this._syncPlacementSelects();
  }

  _syncPlacementSelects() {
    const d = this._devices[this._selectedDevice] || {};
    const areaSel = this.shadowRoot.getElementById("place-area");
    const zoneSel = this.shadowRoot.getElementById("place-zone");
    if (areaSel && d.area_id) areaSel.value = d.area_id;
    if (zoneSel && d.broadcast_zone) zoneSel.value = d.broadcast_zone;
  }

  async _savePlacement() {
    if (!this._selectedDevice) return this._showFeedback("Select a device first", "warn");
    const area_id = this.shadowRoot.getElementById("place-area")?.value || "";
    const broadcast_zone = this.shadowRoot.getElementById("place-zone")?.value || "";
    try {
      await this._hass.connection.sendMessagePromise({
        type: "digital_node_nexus/set_placement",
        device_id: this._selectedDevice,
        area_id,
        broadcast_zone,
      });
      this._devices[this._selectedDevice] = {
        ...this._devices[this._selectedDevice],
        area_id,
        broadcast_zone,
      };
      this._showFeedback("Placement saved", "success");
    } catch (err) {
      this._showFeedback(`Placement failed: ${err.message}`, "error");
    }
  }
  ...
  async _loadDevices() {
    ...
    try {
      const result = await this._hass.connection.sendMessagePromise({
        type: "digital_node_nexus/list_devices",
      });
      (result.devices || []).forEach((d) => {
        this._devices[d.device_id] = d;
      });
      this._renderDeviceList();
      console.info("[Digital Node Nexus] Loaded", Object.keys(this._devices).length, "device(s)");
    } catch (err) {
      console.warn("[Digital Node Nexus] Could not load device list:", err);
    }
  }

  /**
   * Subscribe to digital_node_nexus_device_update HA events.
  ...
  async _subscribeToEvents() {
    ...
    try {
      this._eventUnsubscribe = await this._hass.connection.subscribeEvents(
        ...
        "digital_node_nexus_device_update"
      );
      console.info("[Digital Node Nexus] Subscribed to digital_node_nexus_device_update events");
    } catch (err) {
      console.warn("[Digital Node Nexus] Could not subscribe to device events:", err);
    }
  }

  // â”€â”€â”€ Service Callers â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

  async _callService(service, data) {
    if (!this._hass) return;
    try {
      await this._hass.callService("digital_node_nexus", service, data);
      this._showFeedback(`âœ“ ${service.replace(/_/g, " ")} sent`, "success");
    } catch (err) {
      this._showFeedback(`âœ— Error: ${err.message}`, "error");
    }
  }
  ...
  _render() {
    this.shadowRoot.innerHTML = `
      <link rel="stylesheet" href="/local/digital_node_nexus/alleycat-panel.css">
      <style>
        :host {
          display: block;
      ...
      <div class="layout">
        ...
        <main class="main">
          ...
          <div id="control-panel">
            ...
            <div class="tab-pane" data-tab="haptic" style="display:none">
              <div class="card">
                ...
                <div class="pattern-grid">
                  <button class="pattern-btn" data-pattern="short"><span class="pattern-icon">â–ª</span>Short</button>
                  <button class="pattern-btn" data-pattern="long"><span class="pattern-icon">â–¬</span>Long</button>
                  <button class="pattern-btn" data-pattern="double"><span class="pattern-icon">â–ªâ–ª</span>Double</button>
                  <button class="pattern-btn" data-pattern="sos"><span class="pattern-icon">Â·Â·Â·âˆ’âˆ’âˆ’Â·Â·Â·</span>SOS</button>
                </div>
            ...
            <div class="tab-pane" data-tab="placement" style="display:none">
              <div class="card">
                <p class="card-title">Location and broadcast zone</p>
                <p style="font-size:13px;color:var(--secondary-text-color);margin:0 0 12px">
                  Location is an HA Area (create names in Settings â†’ Areas). Broadcast zone is an AlleycatTV zone id (create in Content Manager). Lists are live ”” nothing is hardcoded.
                </p>
                <label>Location (HA Area)</label>
                <select id="place-area"><option value="">”” none ””</option></select>
                <div style="margin-top:12px">
                  <label>Broadcast zone</label>
                  <select id="place-zone"><option value="">”” none ””</option></select>
                </div>
                <div class="btn-row">
                  <button class="btn btn-primary" id="btn-place-save">Save placement</button>
                  <button class="btn btn-secondary" id="btn-place-reload">Reload lists</button>
                </div>
              </div>
            </div>
  ...
  _attachEventListeners() {
    ...
    root.getElementById("btn-place-save")?.addEventListener("click", () => this._savePlacement());
    root.getElementById("btn-place-reload")?.addEventListener("click", () => this._loadPlacementLists());

    // Broadcast buttons
    root.getElementById("bcast-led-on")?.addEventListener("click", () => {
      Object.keys(this._devices).forEach(id =>
        this._hass?.callService("digital_node_nexus", "set_led", { device_id: id, state: true, brightness: 255, red: 255, green: 255, blue: 255, effect: "solid" })
      );
      this._showFeedback("Broadcast: LEDs ON", "success");
    });
    root.getElementById("bcast-led-off")?.addEventListener("click", () => {
      Object.keys(this._devices).forEach(id =>
        this._hass?.callService("digital_node_nexus", "set_led", { device_id: id, state: false, brightness: 0, red: 0, green: 0, blue: 0, effect: "solid" })
      );
      this._showFeedback("Broadcast: LEDs OFF", "success");
    });
    root.getElementById("bcast-haptic")?.addEventListener("click", () => {
      Object.keys(this._devices).forEach(id =>
        this._hass?.callService("digital_node_nexus", "trigger_haptic", { device_id: id, pattern: "short", intensity: 200, duration_ms: 200, repeat: 1 })
      );
...
customElements.define("digital-node-nexus-panel", ESP32CommanderPanel);
